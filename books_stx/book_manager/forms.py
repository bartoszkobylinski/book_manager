...
class BookForm(forms.ModelForm):
    ...
    def clean_isbn_13(self):
        ...
        if len(str(value)) != 13:
            raise forms.ValidationError(
                "Your ISBN number has not valid length. It should has 13 numbers"
            )
    ...
    def clean_isbn_10(self):
        ...
        if len(str(value)) != 10:
            raise forms.ValidationError(
                "Your ISBN number has not valid length. It should has 10 numbers"
            )
    ...
    def clean_language(self):
        ...
        if value not in get_language_codes():
            raise forms.ValidationError(
                f"""Code given by you is not maching any of possible choices like
        {get_language_codes()}"""
            )
